...
def EnC_Uid(H , Tp):
    ...
    return bytes(e).hex() if Tp == '4177262438' else None
...
def xSEndMsg(Msg , Tp , Tp2 , id , K , V):
    ...
    Pk = "080112" + EnC_Uid(len(Pk) // 2 , Tp = '4177262438') + Pk
...
def GeT_Status(PLayer_Uid , K , V):
    PLayer_Uid = EnC_Uid(PLayer_Uid , Tp = '4177262438')
...
def encrypt_uids():
    global black
    try: 
        if black: black = [EnC_Uid(uid , Tp = '4177262438') for uid in black]
    except: 
        try: open(f, 'w').close()
        except: pass
        load_blacklist()
...
def encrypt_uids2():
    global approve
    try: 
        if approve: approve = [EnC_Uid(uid , Tp = '4177262438') for uid in approve]
    except: 
...
def Add_Black(user_id):
    Add_Uid(user_id)
    if EnC_Uid(user_id , Tp = '4177262438') not in black: black.append(EnC_Uid(user_id , Tp = '4177262438')) ; return True
    else: return False 
    
def Rem_Black(user_id):
    user_id_encrypted = EnC_Uid(user_id , Tp = '4177262438')
...
def Approved(user_id):
    A(user_id)
    if EnC_Uid(user_id , Tp = '4177262438') not in approve: approve.append(EnC_Uid(user_id , Tp = '4177262438')) ; return True
    else: return False 
